...
exports.handler = async function (event) {
  ...
  const { pdfs, recipientEmail, action } = body;
  ...
  if (action === "addLogo") {
    try {
      ...
      return {
        ...
        body: JSON.stringify({ pdfsWithLogo: results }),
      };
    } catch (err) {
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Chyba pri vkladaní loga: " + err.message }),
      };
    }
  }
...
Vráť IBA validné JSON pole, nič iné, žiadny markdown, žiadne \`\`\`.`
      ...
      if (!response.ok) {
        return {
          statusCode: 500,
          body: JSON.stringify({ error: "Claude API chyba: " + JSON.stringify(data) }),
        };
      }
      ...
      // Výpočet súčtu v JavaScripte - nie Claude
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Chyba analýzy: " + err.message }),
      };
    }
  }
  ...
  if (action === "generateEmail") {
    try {
      ...
      const produktyText = produkty
        .map(
          (p, i) =>
            `${i + 1}. ${p.nazov_produktu}: ${p.pocet_ks} ks, ${p.cena_bez_dph.toFixed(2)} EUR bez DPH / ${p.cena_s_dph.toFixed(2)} EUR s DPH`
        )
        .join("\n");
...
- Maximálne 150 slov`,
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Chyba generovania emailu: " + err.message }),
      };
    }
  }
  ...
  if (action === "sendEmail") {
    try {
      ...
      if (!gmailRes.ok) {
        ...
        return {
          statusCode: 500,
          body: JSON.stringify({ error: "Gmail chyba: " + JSON.stringify(err) }),
        };
      }
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Chyba odosielania: " + err.message }),
      };
    }
  }

  return { statusCode: 400, body: JSON.stringify({ error: "Neznáma akcia" }) };
};
